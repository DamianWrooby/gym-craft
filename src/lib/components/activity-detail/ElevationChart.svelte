<script lang="ts">
    ...
    $: areaD = pts.length > 1 ? `${lineD} L ${x(maxTime)} ${HEIGHT - PAD.bottom} L ${PAD.left} ${HEIGHT - PAD.bottom} Z` : '';
</script>
...
{#if pts.length > 1}
    <div class="rounded-xl border border-surface-300 dark:border-surface-700 p-3 bg-surface-100 dark:bg-surface-800 overflow-x-auto">
        <svg viewBox="0 0 {WIDTH} {HEIGHT}" class="w-full h-auto" aria-label="Elevation over time">
            ...
            <text x={PAD.left - 6} y={y(maxE)} class="fill-current text-[10px] opacity-60" text-anchor="end">{Math.round(maxE)} m</text>
            <text x={PAD.left - 6} y={y(minE)} class="fill-current text-[10px] opacity-60" text-anchor="end">{Math.round(minE)} m</text>
        </svg>
    </div>
{:else}
